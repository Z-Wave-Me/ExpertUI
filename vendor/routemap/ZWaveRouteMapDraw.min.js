...
"")}var c=this,p=d3.forceSimulation(this.data.nodes).force("linkForce",d3.forceLink(this.data.links).id(function(a){return a.id}).distance(function(a){return 400}).strength(0));this.data.nodes.map(function(a){a.r=e(a.usage,0,1,15,50,Math.sqrt)/8;a.color=y(a.type)});var k=this.data.nodes.filter(function(a){return null===a.x||null===a.y});if(1===k.length)k[0].x=b.viewport.sizeX/2,k[0].y=b.viewport.sizeY/2;else{var x=Math.min(b.viewport.sizeX,b.viewport.sizeY)/3,q=0;k.forEach(function(a){a.id===c.zna.getMyNodeId()?
a.x=a.y=0:(a.x=x*Math.cos(2*Math.PI/(k.length-1)*q),a.y=x*Math.sin(2*Math.PI/(k.length-1)*q),q++);a.x+=b.viewport.sizeX/2;a.y+=b.viewport.sizeY/2})}var D=this.chartLayer.append("g").attr("class","links").selectAll("line").data(this.data.links).enter().append("line").attr("id1",function(a){return a.source.id}).attr("id2",function(a){return a.target.id}),E=this.chartLayer.append("g").attr("class","nodes").selectAll("circle").data(this.data.nodes).enter().append("circle").attr("id",function(a){return"id"+
...
function(a){return"translate(0,0)"}).call(d3.drag().on("start",m).on("drag",h).on("end",g)).on("mouseover",l).on("mouseout",d).on("click",f);p.on("tick",function(){D.attr("x1",function(a){return a.source.x}).attr("y1",function(a){return a.source.y}).attr("x2",function(a){return a.target.x}).attr("y2",function(a){return a.target.y});E.attr("cx",function(a){return a.x}).attr("cy",function(a){return a.y});F.attr("x",function(a){return a.x}).attr("y",function(a){return a.y})})};return h}();