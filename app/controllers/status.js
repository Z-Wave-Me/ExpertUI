...
appController.controller('StatusController', function ($scope, $filter, $timeout, $interval, dataService, cfg, _, deviceService) {
    ...
    function setData(ZWaveAPIData) {
        ...
        angular.forEach(ZWaveAPIData.devices, function (node, nodeId) {
           ...
           /* if (node.data.genericType.value === 1) {
            ...
            //obj['lastCommunication'] = $filter('getDateTimeObj')(lastCommunication);
    ...
    /**
     * Get sleeping content
     * @param {int} sleepingSince
     * @param {int} lastWakeup
     * @param {int} interval
     * @returns {string}
     */
    function getSleeping(sleepingSince, lastWakeup, interval) {
        var sleeping_cont = '';
        var approx = '';
        ...
        sleeping_cont = '<span title="' + $scope._t('sleeping_since') + '" class="not_important">' + approx + lastSleep + '</span> &#8594; <span title="' + $scope._t('next_wakeup') + '">' + approx + nextWakeup + '</span>';
        //console.log('sleeping_cont',sleeping_cont)
